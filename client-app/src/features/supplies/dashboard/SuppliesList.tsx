...
export default observer(function SuppliesList() {
    ...
    const { suppliesList, newQuantityRegistry, incrementSupplyQuantity, saveSupplyQuantity } = suppliesStore
    return (
        <Grid divided inverted>
            <Grid.Row>
                {suppliesList.map((supply) => (<Grid.Column width='4'>
                    <h3 style={{ color: "cyan" }}>{supply.name}</h3>
                    <Button inverted icon='left chevron' size='mini' onClick={() => incrementSupplyQuantity(supply.id, false)} />
                    <span style={{ fontSize: "1.5em", paddingInline: "10px" }}>
                        {newQuantityRegistry.get(supply.id)}{supply.denomination} </span>
                    <Button inverted icon='right chevron' size='mini' onClick={() => incrementSupplyQuantity(supply.id, true)} />
                    <span style={{ paddingInline: "10px" }}>
                        <Button
                            disabled={supply.quantity === newQuantityRegistry.get(supply.id)}
                            inverted icon='save' size='mini'
