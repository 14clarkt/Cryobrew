...
{
    public class CreateAlchemyIngredient
    {
        ...
        public class Handler : IRequestHandler<Command, Result<Unit>>
        {
            ...
            {
                _context.AlchemyIngredients.Add(request.AlchemyIngredient);

                var result = await _context.SaveChangesAsync() > 0;

                if (!result) return Result<Unit>.Failure("Failed to create AlchemyIngredient");
